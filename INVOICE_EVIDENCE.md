...
The article and tables are derived from the recorded inputs, probabilities and scores preserved here. Model files, the original sealed benchmark result and private Primus training methods are unchanged by this documentation publication. The archive article and evidence guide follow the current explanatory copy. Original JSON records, protocols, predictions and evaluation scripts remain byte-identical; [the online case study](INVOICE_CASE_STUDY.md) presents the same results.
